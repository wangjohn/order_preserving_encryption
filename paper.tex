...
\newenvironment{thm}{{\bfseries Theorem:}}
\newenvironment{lemma}{{\bfseries Lemma:}}
\newenvironment{proof}{{\bfseries Proof:}}{\qedsymbol}
...
Definition: The adversary's advantage is the difference $|Pr[\textrm{win}^{Adv, \kappa, \lambda}] - 1/2|$.

Definition: IND-CPDA. A mOPE scheme is IND-CPDA secure if for all adversaries and all sufficiently large $\kappa$ and $\lambda$, the adversary's advantage is negligible, i.e. that $Pr[\textrm{win}^{Adv, \kappa, \lambda}] \leq 1/2 + \textrm{negl}(\kappa, \lambda)$.
